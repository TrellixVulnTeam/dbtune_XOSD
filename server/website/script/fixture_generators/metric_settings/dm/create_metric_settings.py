...
def create_settings(metric_data, dbms):
    ...
    for name, value in metric_data.items():
        ...
        if vartype in (2, 3):
            mettype = 3 #STATISTICS
        else:
            if name.endswith('wait_class#'):
...
def usage():
    print('python3 create_metric_settings.py [version] (valid versions: {})'.format(', '.join(VERSIONS)))
    sys.exit(1)
