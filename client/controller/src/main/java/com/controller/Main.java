    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (time < 0) {
                ...
                int pid = Integer.parseInt(vmName.substring(0, p));
                try {
                    f.createNewFile();
    ...
    private static DBCollector getCollector(ControllerConfiguration config) {
        ...
        switch (config.getDBType()) {
            case POSTGRES:
                ...
                break;
            case MYSQL:
                collector = new MySQLCollector(config.getDBURL(), config.getDBUsername(), config.getDBPassword());
                break;
            case SAPHANA:
                collector = new SAPHanaCollector(config.getDBURL(), config.getDBUsername(), config.getDBPassword());
                break;
            case ORACLE:
                collector = new OracleCollector(config.getDBURL(), config.getDBUsername(), config.getDBPassword());
                break;
            case DM:
                collector = new DmCollector(config.getDBURL(), config.getDBUsername(), config.getDBPassword());
                break;
            default:
