#!/bin/bash

db_name=${DB_NAME:-'db_tune'}
db_password=${DB_PASSWORD:-'MTIzNDU2NzgK'}
pwd=$(echo $db_password | base64 -d)

chown -R mysql:mysql /var/lib/mysql
#ps -ef | grep "mysql" | grep -v grep | tr -s " " | cut -d" " -f2 | xargs kill -9
netstat -anp | grep "mysql" &> /dev/null
if [ $? -ne 0 ]; then
  	echo "start mysql service..."
    nohup mysqld_safe --defaults-file=/var/lib/mysql/conf/my.cnf >/dev/null 2>&1 &
    if [ "$?" -ne 0 ]; then
      echo "mysql service not ready..."
      exit 0
    fi
fi


ready() {
  a=$(netstat -nlp | grep 3306 | wc -l)
  if [ "$a" -gt "0" ]; then
    return 0
  else
    return 1
  fi
}

counter=1
max_counter=30
while ! ready; do
  counter=$(expr $counter + 1)
  sleep 1

  if [ $counter -gt $max_counter ]; then
    echo "ERROR: Could not connect to mysql; Exiting..."
    exit 1
  fi
  sleep 1
done

sleep 3

init=$(mysql --host="localhost" --protocol TCP -u"root" -p"$pwd" -s -Ne "SELECT
                                                                  count(*) num
                                                              FROM
                                                                  information_schema.TABLES
                                                              WHERE
                                                                 table_schema = '$db_name';")
if [ "$init" = "" ] || [$(echo "$init" | awk 'NR==1{print}') -eq 0]; then
echo "mysql install table..."
# 本地首次使用sock文件登录mysql是不需要密码的
mysql -S /var/lib/mysql/tmp/mysqld.sock <<EOF
use mysql;
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '$pwd';
grant all privileges on *.* to 'root'@'localhost';
# 刷新权限表
FLUSH PRIVILEGES;
CREATE USER 'root'@'%' IDENTIFIED BY '$pwd';
# 修改密码
ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY '$pwd';
grant all privileges on *.* to 'root'@'%';
# 刷新权限表
FLUSH PRIVILEGES;
select host,user,authentication_string from mysql.user;
create database $db_name default character set utf8mb4 collate utf8mb4_unicode_ci;
EOF

echo "mysql install table success..."
fi

# 登录sock软连接到tmp目录
ln -fs /var/lib/mysql/tmp/mysqld.sock /tmp/mysql.sock

echo "-=------------------------------------------------------"
echo "-=---------------  数据库列表： ---------------------------"
echo "$(mysql --host="localhost" --protocol TCP -u"root" -p"$pwd" -e "show databases;")"
echo "-=------------------------------------------------------"

mysql_res=$(mysql --host="localhost" --protocol TCP -u"root" -p"$pwd" -s -Ne "SELECT
                                                                  count(*) num
                                                              FROM
                                                                  information_schema.TABLES
                                                              WHERE
                                                                  table_schema = '$db_name';")
res=$(echo "$mysql_res" | awk 'NR==1{print}')
export IS_DB_INIT="$res"
